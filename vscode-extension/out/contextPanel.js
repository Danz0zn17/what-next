...
exports.ContextCardViewProvider = void 0;
...
function getWebviewContent(project, orientation, card, error) {
    ...
    const stackPills = intel?.stack
        ? intel.stack.split(',').map((s) => `<span class="pill">${esc(s.trim())}</span>`).join('')
        : '';
    ...
    const taskLines = tasks
        ? tasks.split('\n').filter(Boolean).map(t => `<div class="task-row"><span class="task-bullet">·</span><span>${esc(t.replace(/^[-*]\s*/, ''))}</span></div>`).join('')
        : '';
    ...
    async refresh() {
        ...
        try {
            const [orientation, card] = await Promise.all([
                (0, api_1.getOrientation)(project).catch(() => null),
                (0, api_1.getContextCard)(project).catch(() => null),
            ]);
...
exports.ContextCardViewProvider = ContextCardViewProvider;
